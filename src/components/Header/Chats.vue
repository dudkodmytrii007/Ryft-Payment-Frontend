<script lang="ts" setup>
  ...
  const filteredChats = computed(() => {
    return myChats.value.filter(chat => {
      ...
      const matchesType = currentChatType.value === 'all' || chat.chatType === currentChatType.value;
...
<template>
  <div class="chats">
    <div class="chat-type">
      <button @click="changeChatType('all')" :class="{ active: currentChatType === 'all' }">All</button>
      <button @click="changeChatType('private')" :class="{ active: currentChatType === 'private' }">Messages</button>
      <button @click="changeChatType('group')" :class="{ active: currentChatType === 'group' }">Groups</button>
    </div>

    <SearchBar v-model="searchQuery" placeholder="Search for your chat" class="search-bar" />
...
<style scoped>
  ...
  .chat-type button:hover::after, .chat-type .active::after {
    transform: scaleX(1);
    height: 2px;
  }
